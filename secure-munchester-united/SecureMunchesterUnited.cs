public class SecurityPassMaker
{
    public string GetDisplayName(TeamSupport support)
    {
        if (support is Staff)
        {
            bool isNotSecurityJunior = support.GetType() != typeof(SecurityJunior);
            bool isNotSecurityInter = support.GetType() != typeof(SecurityIntern);
            bool isNotPoliceLiaison = support.GetType() != typeof(PoliceLiaison);

            if (support is Security && isNotSecurityJunior && isNotSecurityInter && isNotPoliceLiaison)
            {
                return string.Concat(support.Title, " Priority Personnel");
            }
